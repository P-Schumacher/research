...
def main(cnf):
    ...
    sim_avg = np.zeros([N,], dtype=np.float32) 
    td_avg = np.zeros([N,], dtype=np.float32) 
    for idx, s in enumerate(state):
        ...
        sim_avg[idx] += sims
        td_avg[idx] += td_errors
    sim = sim_avg / SAMPLES
    td = td_avg / SAMPLES
    np.save(f'sim_avg.npy', sim)
    np.save(f'td_avg.npy', td)
